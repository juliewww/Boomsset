...
品牌色**墨蓝 `#1F4E85`**。之前没有品牌色 —— 全局一句裸 `MaterialTheme {}`，
跑的是 Material 3 自带的默认紫（淡紫 FAB、紫色导航指示条都是从那来的）。
...
3. **换图标后 SpringBoard 有一瞬间的深色占位态。** 截到的是深灰底、对比度很差的一张，
   看着像 iOS 自动派生的深色变体出了问题 —— 其实只是缓存还没刷新，等几秒重截就正常。
   排查图标问题前先确认不是这个。
