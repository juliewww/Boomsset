...
**多币种已打通（实机验证）：** 汇率走 Ktor + Frankfurter（ECB 数据，无需 key，
**支持历史日期**），基准币种可切换且持久化。实测 $1000 → ¥6,771.30，切换口径后
快照表一条都没被改写。

**还没做的：** **股票/基金行情源**（需要 API key，未定供应商 —— 所以 `QUOTED` 资产
仍无法在 UI 创建）、目标配置编辑、资产元信息编辑、应用锁。
`iosApp/` 缺 .xcodeproj（iOS 从未运行过），见 iosApp/README.md。

**教训（三次都是实跑才发现、编译和单测全绿）：**
