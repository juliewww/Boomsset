...
**品牌色与 app icon 已做。⚠️ 当前值是橄榄金 `#918163`** —— 定义在
[Theme.kt](shared/src/commonMain/kotlin/com/boomsset/ui/theme/Theme.kt)，跟随系统深浅色。
下面几段按时间顺序记录了 `#8A5A18` → `#BD4D03` → `#918163` 的两次改版和各自被否的理由，
**读到旧色号的段落时注意那是历史，不是现状。**
图标是「破环而出」（五色配置环 + 三根上升柱，最高一根穿出环外），全部尺寸由
...
* **冷色被否，但原因是可计算的**：五个大类色里**四个是冷色**，在亮度轴上把冷色区
  分段占满了（保障类紫 L.48、流动资金蓝 L.60、固定收益绿和另类实物青 L.70）。
...
判据是**对它实际压在的那块底**有 ≥4.5:1，而最不利的底是 hero 卡片（比页面底深）——
旧值 `#C5453F` 压在新的暖沙卡片上只有 **3.34:1**。现在由 `BoomssetTheme` 通过
...
现在选中态走**两个通道**：`onPrimaryContainer` 深棕（12.72:1）+ `SemiBold`。
同时把指示条设成透明 —— M3 把它画在**图标位**，而这一栏 `icon = {}` 没有图标，
它就是个空的圆角块，默认色 `secondaryContainer` 压在导航栏底上又只有 1.05:1。
...
⚠️ **这一轮没验到的：** 净值页 hero 卡片**装着真实数据时的样子**（手上模拟器里
4 项资产全是已归档状态，净值页走空状态分支）；**深色模式没上设备看**；
...
| 配色 | **莫兰迪暖色**：品牌橄榄金 `#918163`（OKLCH H=82°，彩度仅 0.047），表面是同色相微染的暖 greige。全部色值由这一个色相角解出，**图标也是** |
