...
**品牌色与 app icon 已做。⚠️ 当前值是暖金 `#986E00`** —— 定义在
[Theme.kt](shared/src/commonMain/kotlin/com/boomsset/ui/theme/Theme.kt)，跟随系统深浅色。
下面几段按时间顺序记录了 `#8A5A18` → `#BD4D03` → `#918163` → `#986E00` 的三次改版和各自被否的理由，
...
✅ 净值页 hero 卡片**装真实数据的样子已经验过了**（模拟器里录入资产后截图确认）。
⚠️ **仍然没验到的：** **深色模式没上设备看**；
...
| 配色 | 品牌**暖金** `#986E00`（OKLCH H=82°、L 0.565、彩度 0.120），表面是同色相微染的暖 greige。全部色值由这一个色相角解出，**图标也是** |
