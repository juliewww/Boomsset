...
# ── 设计常量 ──────────────────────────────────────────────────────────
# 品牌色相角。⚠️ 必须和 shared/.../ui/theme/Theme.kt 里的 BrandCream (#D3BC7D) 一致 ——
...
FIELD = (249, 243, 227)         # #F9F3E3，= OKLCH(0.965, 0.022, 90)
...
# 2b. **柱与任一环段的 ΔE ≥ 10**（实测 10.5，最接近的一对是最短那根柱 ↔ 权益类橙）。
#    柱和环之间虽然有底色间隙隔开，但色值太接近时会读成"柱子是环的一部分"。
#    ⚠️ 金柱和权益类橙天生接近，**这条一直是最紧的一条**，改柱色先量它。
# 3. **每段对底 ≥ 1.8:1**（实测 2.20~5.45）。这条反过来决定了 FIELD，见上面
RING = [
    (0.26, (88, 92, 162)),      # #585CA2  保障类    L 0.505  对底 5.45:1
...
# 三根上升柱走**品牌奶黄**（色相 90°）—— 环已经是五个色相了，柱子再多色会吵。
# 中间那根 ≈ `ChartColors.brand`（`#9D8B5A`，界面上柱子和导航指示条用的那个）。
#
# ⚠️ **柱子不能再亮/再艳了**：判据 2b（柱与任一环段 ΔE ≥ 10）把它压在这里 ——
# 「权益类」橙 `#E58A26` 坐在 L 0.715，金色系一往那个亮度走就撞上（试过 L 0.70
# 那档，只有 8.5）。现在实测 10.5，贴着门槛。
#
# ⚠️ **不要再往深里调。** 最早是 L 0.50/0.43/0.265，反馈"太沉闷" —— 根因是
# 八个元素全都比底色暗，最高那根柱（近乎黑的 #322200）是最大的单块暗部。
BARS = [(174, 159, 119), (151, 136, 95), (129, 114, 73)] # #AE9F77 #97885F #817249
