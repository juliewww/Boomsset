...
# ── 设计常量 ──────────────────────────────────────────────────────────
# 品牌色相角。⚠️ 必须和 shared/.../ui/theme/Theme.kt 里的 BrandGold (#955E00) 一致 ——
...
FIELD = (253, 241, 228)         # #FDF1E4，= OKLCH(0.965, 0.022, 70)
...
# 三根上升柱走**品牌金**（色相 82°）—— 环已经是五个色相了，柱子再多色会吵。
# **中间那根就是 Theme.kt 的 `BrandGold`（#955E00）**，另两根是它的明度上下阶。
# 图标和界面因此共用同一个金，不是"看起来差不多"。
#
# ⚠️ **不要再往深里调。** 最早是 L 0.50/0.43/0.265，反馈"太沉闷" —— 根因是
# 八个元素全都比底色暗，最高那根柱（近乎黑的 #322200）是最大的单块暗部。
BARS = [(170, 115, 43), (149, 94, 0), (123, 76, 0)]     # #AA732B #955E00 #7B4C00
