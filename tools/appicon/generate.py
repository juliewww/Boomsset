...
#    实测 8.4。段序**不是**大类展示顺序，是在 120 种排列里搜出的 ——
#    按大类顺序排只有 6.1，蓝/青和绿/橙在色盲下会并到一起。
#    图标不是图例，段序不承载语义，可以为可读性让路。
#
# 2b. **柱与任一环段的 ΔE ≥ 10**（实测 14.3）。柱和环之间虽然有底色间隙隔开，
#    但色值太接近时会读成"柱子是环的一部分"。段序是搜出来的，所以这条也是
#    搜索条件之一，不是事后手调。
#
#    ⚠️ **这条决定了「权益类」为什么坐在最暗的一位。** 柱子是金色系，而权益类
#    本身就是橙色 —— 只要它落在环的亮端（L 0.622），和金柱的 ΔE 就只有 3.5，
#    读起来像柱子是从环上掉下来的一块。试过 8 个柱亮度/彩度组合，**没有一个
#    能在不动段序的前提下过关**（3.5～8.6）。所以把它挪到最暗位。
#
...
RING = [
    (0.26, (91, 49, 0)),        # #5B3100  权益类    62°    L 0.360  对底 5.79:1
    (0.22, (25, 80, 133)),      # #195085  流动资金 251°    L 0.426  对底 4.34:1
    (0.20, (0, 108, 131)),      # #006C83  另类实物 219°    L 0.491  对底 3.24:1
    (0.17, (25, 135, 99)),      # #198763  固定收益 165°    L 0.557  对底 2.44:1
    (0.15, (122, 128, 194)),    # #7A80C2  保障类   279°    L 0.622  对底 1.98:1
]
GAP_DEG = 1.6                   # 段间留白，让"分段"读得出来

# 三根上升柱走**金色系**（色相 78°，彩度 0.105）—— 环已经是五个色相了，
# 柱子再多色会吵；金色和底色同宗，所以柱子读起来是"底色长出来的"。
# 最高那根最深（穿出环外，是整个图标的落点）。
#
# ⚠️ **不要再往深里调。** 最早是 L 0.50/0.43/0.265，反馈是"太沉闷" ——
# 根因是**八个元素全都比底色暗**，而最高那根柱（近乎黑的 #322200）是最大的
# 单块暗部。中间一版提到 L 0.52/0.46/0.385（暖褐），反馈仍然偏沉，
# 而且"底色那圈金最好看"—— 所以柱子干脆并进金色系，现在 L 0.63/0.56/0.50。
#
# ⚠️ **也不要再往亮里调。** 最短那根柱对底只有 1.88:1，已经贴着"看得见"的下限；
# 再亮一档（L 0.54）就掉到 1.60:1，柱子会糊进底色。
BARS = [(172, 129, 55), (152, 109, 32), (131, 90, 0)]   # #AC8137 #986D20 #835A00
